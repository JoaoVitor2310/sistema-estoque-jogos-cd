...
- **Toda entrada/saída carrega uma categoria.** `income` exige um `IncomeCategory` (Saque Gamivo, Investimento externo, Rendimentos ou Outros); `expense` exige um `ExpenseCategory` (Compra de Jogo, Impostos, Assinaturas ou Outros) — enum fixo, `Outros` sempre disponível como fallback. É um campo à parte de `description`: a categoria classifica para agrupar depois, a descrição segue livre para o detalhe pontual.
