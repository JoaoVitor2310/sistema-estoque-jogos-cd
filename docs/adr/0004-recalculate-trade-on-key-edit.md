...
- **A trade não é excluída após importar.** Como as keys referenciam `trades.id` e a FK é `nullOnDelete`, apagar a trade zeraria o `trade_id` das keys — anulando o vínculo. Por isso importar as keys (sem erros) marca `trades.is_imported = true` em vez de excluir; a aba de Trades lista só trades não importadas, então a importada some da tela mas fica no banco. Ver o campo `is_imported` em `CLAUDE.md`.
