...
<template>
  ...
  <ConfirmPopup />
  <Dialog v-model:visible="DialogVisible" modal :header="isEdit ? 'Editar' : 'Criar'" :style="{ width: '80rem', paddingBottom: '5rem' }"
    maximizable>
    ...
    <div v-for="(item, index) in selected" class="d-flex flex-row gap-2">
      ...
      <div class="d-flex flex-column">
        ...
        <div class="d-flex gap-5 mb-3">
          <Select v-model="item.tipo_reclamacao_id" :options="props.tiposReclamacao" optionLabel="name" optionValue="id"
      ...
      <div class="d-flex flex-column">
        ...
        <div class="d-flex gap-5 mb-3">
          <DatePicker v-model="item.dataVendida" dateFormat="dd/mm/yy" showIcon fluid :showOnFocus="false" showButtonBar
            style="min-width: 10rem" />
        </div>
      </div>
      <div class="d-flex flex-column">
  ...
  <div class="text-center mb-3 mx-5">
    ...
    <DataTable :value="rowData" showGridlines resizableColumns reorderableColumns sortMode="multiple" removableSort
      ...
      <Column field="tipo_formato.name" header="Formato" filterField="searchField" :showFilterMenu="true"
        ...
        <template #editor="{ data, field }">
          <Select v-model="data.tipo_formato.id" :options="props.tiposFormato" @change="onEdit(data)" optionLabel="name"
            optionValue="id" />
        </template>
      </Column>
      ...
      <Column field="leilao_gamivo.name" header="Leilão Gamivo" class="text-center p-0">
        ...
        <template #editor="{ data, field }">
          <Select v-model="data.leilao_gamivo.id" :options="props.tiposLeilao" @change="onEdit(data)" optionLabel="name"
            optionValue="id" />
        </template>
      </Column>
      <Column field="leilao_kinguin.name" header="Leilão Kinguin" class="text-center p-0">
        ...
        <template #editor="{ data, field }">
          <Select v-model="data.leilao_kinguin.id" :options="props.tiposLeilao" @change="onEdit(data)" optionLabel="name"
            optionValue="id" />
        </template>
      </Column>
      ...
      <Column field="leiloes" header="Leilões" sortable class="text-center p-0">
        <template #editor="{ data, field }">
          <InputNumber v-model="data[field]" @change="onEdit(data)" mode="decimal" :min="0" useGrouping autofocus fluid />
        </template>
      </Column>
