...
const props = defineProps({ assets: Array as PropType<Resource[]> });
Object.assign(rowData, props.assets);
// }
...
const onEdit = async (product: Resource, currentCurrency: string | null = null) => {
  isEdit.value = true;
  const res = await axiosInstance.put(`/assets/${product.id}`, {
...
const onAdd = async (newAsset: Resource): Promise<void> => { // Faz a req pra api add o elemento
  try {
    const res = await axiosInstance.post(`/assets`, {
      name: newAsset.name,
      price_euro: newAsset.price_euro,
      price_dollar: newAsset.price_dollar,
      price_brl: newAsset.price_brl,
    });
    showResponse(res, toast.add);
    if (res.status === 201) {
      DialogVisible.value = false;
  ...
  confirm.require({
    ...
    accept: async () => {
      if (qtd === 1) {
        ...
        const res = await axiosInstance.delete(`/assets`, {
          data: {
            assets: selectedProduct.value.map(item => ({ id: item.id }))
          }
        });
        showResponse(res, toast.add);
